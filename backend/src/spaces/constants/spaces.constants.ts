...
export const SPACE_ERROR_MESSAGES = {
  ...
  PERMISSION_DENIED:
    'You do not have permission to perform this action on this space',
  OWNER_KEY_SLOT_REQUIRED: 'Owner key slot is required for shared spaces',
  OWNER_KEY_SLOT_FORBIDDEN:
    'Owner key slot must not be provided for personal spaces',
  ...
  NOTE_KEY_REQUIRED_FOR_PERSONAL:
    'Encrypted note key is required for personal space notes',
  NOTE_KEY_FORBIDDEN_FOR_SHARED:
    'Encrypted note key must not be provided for shared space notes',
} as const;
